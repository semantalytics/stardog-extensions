...
public class TestDetectFromAllExcept extends AbstractStardogTest {
    ...
    public void testDetectAllEnglishPhrase() {
        ...
        final String aQuery = LanguageVocabulary.sparqlPrefix("lang") +
                    "select ?result where { bind(lang:detectFromAllExcept(\"Stardog graph database\", \"german\") AS ?result) }";
    ...
    public void testDetectAllGermanPhrase() {
        ...
        final String aQuery = LanguageVocabulary.sparqlPrefix("lang") +
                "select ?result where { bind(lang:detectFromAllExcept(\"guten morgan\", \"german\") AS ?result) }";
        ...
        try (final SelectQueryResult aResult = connection.select(aQuery).execute()) {
            ...
            assertThat(assertStringLiteral(aValue));
            final Literal aLiteral = ((Literal)aValue);
            assertThat(aLiteral.toString()).isEqualTo("\"guten morgan\"@nn");
            assertThat(aResult).isExhausted();
        }
    }
