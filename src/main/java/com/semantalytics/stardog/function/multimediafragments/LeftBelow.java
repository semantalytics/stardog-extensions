...
public class LeftBelow extends AbstractFunction implements UserDefinedFunction {

    public LeftBelow() {
        super(2, Constants.NAMESPACE + "leftBelow");
    }

    public LeftBelow(final LeftBelow leftBelow) {
        super(leftBelow);
    }
        ...
        if (Entities.haveComparable(values)) {
            try {
                ...
                if(entity1.isPresent() && entity2.isPresent()) {
                    return ValueOrError.Boolean.of(SpatialFunction.DirectionalFunction.leftBelow(entity1.get(), entity2.get()));
                } else {
