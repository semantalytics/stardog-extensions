...
public class SpatialOverlaps extends AbstractFunction implements UserDefinedFunction {

    public SpatialOverlaps() {
        super(2, Constants.NAMESPACE + "spatialOverlaps");
    }

    public SpatialOverlaps(SpatialOverlaps spatialOverlaps) {
        super(spatialOverlaps);
    }
        ...
        if(Entities.haveComparable(values)) {
            try {
                ...
                if(entity1.isPresent() && entity2.isPresent()) {
                    return ValueOrError.Boolean.of(SpatialFunction.TopologicalFunction.crosses(entity1.get(), entity2.get()));
                } else {
    ...
    @Override
    public SpatialOverlaps copy() {
        return new SpatialOverlaps(this);
    }
