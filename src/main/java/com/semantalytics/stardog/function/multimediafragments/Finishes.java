...
public class Finishes extends AbstractFunction implements UserDefinedFunction {

    public Finishes() {
        super(2, Constants.NAMESPACE + "finishes");
    }

    public Finishes(final Finishes finishes) {
        super(finishes);
    }
        ...
        if(Entities.haveComparable(values)) {
            try {
                ...
                if(entity1.isPresent() && entity2.isPresent()) {
                    return ValueOrError.General.of(literal(TemporalFunction.finishes(entity1.get(), entity2.get())));
                } else {
    ...
    @Override
    public Finishes copy() {
        return new Finishes(this);
    }
