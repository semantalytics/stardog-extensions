...
public class OverlappedBy extends AbstractFunction implements UserDefinedFunction {

    public OverlappedBy() {
        super(2, Constants.NAMESPACE + "overlappedBy");
    }

    public OverlappedBy(final OverlappedBy overlappedBy) {
        super(overlappedBy);
    }
        ...
        if(Entities.haveComparable(values)) {
            try {
                ...
                if(entity1.isPresent() && entity2.isPresent()) {
                    return ValueOrError.General.of(literal(TemporalFunction.overlappedBy(entity1.get(), entity2.get())));
                } else {
    ...
    @Override
    public OverlappedBy copy() {
        return new OverlappedBy(this);
    }
