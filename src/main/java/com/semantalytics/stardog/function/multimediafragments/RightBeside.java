...
public class RightBeside extends AbstractFunction implements UserDefinedFunction {

    public RightBeside() {
        super(2, Constants.NAMESPACE + "rightBeside");
    }

    public RightBeside(final RightBeside rightBeside) {
        super(rightBeside);
    }
        ...
        if(Entities.haveComparable(values)) {
            try {
                ...
                if(entity1.isPresent() && entity2.isPresent()) {
                    return ValueOrError.Boolean.of(SpatialFunction.DirectionalFunction.rightBeside(entity1.get(), entity2.get()));
                } else {
    ...
    @Override
    public RightBeside copy() {
        return new RightBeside(this);
    }
