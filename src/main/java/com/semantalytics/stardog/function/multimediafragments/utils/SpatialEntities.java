...
public class SpatialEntities {
    ...
    public static Optional<SpatialEntity> of(Value v) throws MediaFragmentURISyntaxException, ParseException {
        if(v instanceof IRI) {
            return SpatialEntities.of((IRI)v);
        } else if(v instanceof Literal) {
            return SpatialEntities.of((Literal)v);
        } else {
            return Optional.empty(); //not the best but it will do. It's not really optional but ok
        }
    }
