...
public class MetBy extends AbstractFunction implements UserDefinedFunction {

    public MetBy() {
        super(2, Constants.NAMESPACE + "metBy");
    }

    public MetBy(final MetBy metBy) {
        super(metBy);
    }
        ...
        if(Entities.haveComparable(values)) {
            try {
                ...
                if(entity1.isPresent() && entity2.isPresent()) {
                    return ValueOrError.General.of(literal(TemporalFunction.metBy(entity1.get(), entity2.get())));
                } else {
