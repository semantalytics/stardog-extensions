...
public final class UniformDatatype extends AbstractExpression implements UserDefinedFunction {

    public UniformDatatype() {
        super(new Expression[0]);
    }

    public UniformDatatype(final UniformDatatype uniformDatatype) {
        super(uniformDatatype);
    }
    ...
    public ValueOrError evaluate(final ValueSolution valueSolution) {
        ...
        if(getArgs().size() == 1) {
           ...
           if(!firstArgValue.isError() && assertArrayLiteral(firstArgValue.value())) {
               ...
               if(isUniformLiteral) {
                    return ValueOrError.General.of(((Literal)values.get(0)).datatypeIRI());
               } else {
    ...
    @Override
    public UniformDatatype copy() {
        return new UniformDatatype(this);
    }
