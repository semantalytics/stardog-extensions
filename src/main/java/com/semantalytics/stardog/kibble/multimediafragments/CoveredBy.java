...
public class CoveredBy extends AbstractFunction implements UserDefinedFunction {

    public CoveredBy() {
        super(2, Constants.NAMESPACE + "coveredBy");
    }

    public CoveredBy(final CoveredBy coveredBy) {
        super(coveredBy);
    }
        ...
        if (Entities.haveComparable(values)) {
            try {
                ...
                if (entity1.isPresent() && entity2.isPresent()) {
                    return ValueOrError.Boolean.of(SpatialFunction.TopologicalFunction.coveredBy(entity1.get(), entity2.get()));
                } else {
    ...
    @Override
    public CoveredBy copy() {
        return new CoveredBy(this);
    }
