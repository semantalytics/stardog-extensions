...
public class TemporalMeets extends AbstractFunction implements UserDefinedFunction {

    public TemporalMeets() {
        super(2, Constants.NAMESPACE + "temporalMeets");
    }

    public TemporalMeets(final TemporalMeets temporalMeets) {
        super(temporalMeets);
    }
        ...
        if(Entities.haveComparable(values)) {
            try {
                ...
                if(entity1.isPresent() && entity2.isPresent()) {
                    return ValueOrError.General.of(literal(TemporalFunction.meets(entity1.get(), entity2.get())));
                } else {
    ...
    @Override
    public TemporalMeets copy() {
        return new TemporalMeets(this);
    }
