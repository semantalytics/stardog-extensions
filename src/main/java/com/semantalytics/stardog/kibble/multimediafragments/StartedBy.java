...
public class StartedBy extends AbstractFunction implements UserDefinedFunction {

    public StartedBy() {
        super(2, Constants.NAMESPACE + "startedBy");
    }

    public StartedBy(final StartedBy startedBy) {
        super(startedBy);
    }
        ...
        if(Entities.haveComparable(values)) {
            try {
                ...
                if(entity1.isPresent() && entity2.isPresent()) {
                    return ValueOrError.General.of(literal(TemporalFunction.startedBy(entity1.get(), entity2.get())));
                } else {
    ...
    @Override
    public StartedBy copy() {
        return new StartedBy(this);
    }
