...
public class SpatialContains extends AbstractFunction implements UserDefinedFunction {

    public SpatialContains() {
        super(2, Constants.NAMESPACE + "spatialContains");
    }

    public SpatialContains(final SpatialContains spatialContains) {
        super(spatialContains);
    }
        ...
        if(Entities.haveComparable(values)) {
            try {
                ...
                if(entity1.isPresent() && entity2.isPresent()) {
                    return ValueOrError.Boolean.of(SpatialFunction.TopologicalFunction.contains(entity1.get(), entity2.get()));
                } else {
    ...
    @Override
    public SpatialContains copy() {
        return new SpatialContains(this);
    }
