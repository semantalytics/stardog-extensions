...
public class RightAbove extends AbstractFunction implements UserDefinedFunction {

    public RightAbove() {
        super(2, Constants.NAMESPACE + "rightAbove");
    }

    public RightAbove(final RightAbove rightAbove) {
        super(rightAbove);
    }
        ...
        if(Entities.haveComparable(values)) {
            try {
                ...
                if(entity1.isPresent() && entity2.isPresent()) {
                    return ValueOrError.Boolean.of(SpatialFunction.DirectionalFunction.rightAbove(entity1.get(), entity2.get()));
                } else {
    ...
    @Override
    public RightAbove copy() {
        return new RightAbove(this);
    }
