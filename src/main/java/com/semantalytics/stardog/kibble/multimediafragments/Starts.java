...
public class Starts extends AbstractFunction implements UserDefinedFunction {

    public Starts() {
        super(2,Constants.NAMESPACE + "starts");
    }

    public Starts(final Starts starts) {
        super(starts);
    }
        ...
        if(Entities.haveComparable(values)) {
            try {
                ...
                if(entity1.isPresent() && entity2.isPresent()) {
                    return ValueOrError.General.of(literal(TemporalFunction.starts(entity1.get(), entity2.get())));
                } else {
