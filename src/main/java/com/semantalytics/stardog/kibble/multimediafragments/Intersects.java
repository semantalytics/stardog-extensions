...
public class Intersects extends AbstractFunction implements UserDefinedFunction {

    public Intersects() {
        super(2, Constants.NAMESPACE + "intersects");
    }

    public Intersects(final Intersects intersects) {
        super(intersects);
    }
        ...
        if(Entities.haveComparable(values)) {
            try {
                ...
                if(entity1.isPresent() && entity2.isPresent()) {
                    return ValueOrError.Boolean.of(SpatialFunction.TopologicalFunction.intersects(entity1.get(), entity2.get()));
                } else {
