...
public final class SplitPreserveAllTokens extends AbstractExpression implements StringFunction {

    protected SplitPreserveAllTokens() {
        super(new Expression[0]);
    }

    private SplitPreserveAllTokens(final SplitPreserveAllTokens split) {
        super(split);
    }
    ...
    @Override
    public SplitPreserveAllTokens copy() {
        return new SplitPreserveAllTokens(this);
    }
        ...
        if(getArgs().size() <= 3 && getArgs().size() >= 2) {
            ...
            if (!separatorValueOrError.isError() && assertStringLiteral(separatorValueOrError.value())) {
                ...
                if (getArgs().size() == 3) {
                    ...
                    splits = splitPreserveAllTokens(string, separator, max);
                } else {
                    splits = splitPreserveAllTokens(string, separator);
                }
        } else {
            return ValueOrError.Error;
        }
