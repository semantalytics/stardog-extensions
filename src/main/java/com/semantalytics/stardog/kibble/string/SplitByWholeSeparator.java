...
public final class SplitByWholeSeparator extends AbstractExpression implements StringFunction {

    protected SplitByWholeSeparator() {
        super(new Expression[0]);
    }

    private SplitByWholeSeparator(final SplitByWholeSeparator split) {
        super(split);
    }
    ...
    @Override
    public SplitByWholeSeparator copy() {
        return new SplitByWholeSeparator(this);
    }
        ...
        if(getArgs().size() <= 3 && getArgs().size() >= 2) {
            ...
            if (!separatorValueOrError.isError() && assertStringLiteral(separatorValueOrError.value())) {
                ...
                if (getArgs().size() == 3) {
                    ...
                    splits = splitByWholeSeparator(string, separator, max);
                } else {
                    splits = splitByWholeSeparator(string, separator);
                }
        } else {
            return ValueOrError.Error;
        }
