...
public class Sha256 implements RDFExtractor {
    ...
    public StatementSource extract(Connection theConnection, IRI theDocIri, Path theDocContents) throws BitesException {
        try {
            if (theDocContents.toFile().length() > 0L) {
                HashCode hashCode = Hashing.sha256().newHasher().putBytes(Files.readAllBytes(theDocContents)).hash();
                Set<Statement> aModel = new HashSet();
                aModel.add(Values.statement(theDocIri, Values.iri("http://stardog.semantalytics.com/2019/11/ns/hash/sha256"), Values.literal(hashCode.toString())));
